...
class LibPSGCProvinces extends Seeder
{
    protected $excelFilePath = 'public/system-db-libraries/lib_psgc_provinces.xls'; // Set the path to your Excel file
    protected $table = 'lib_psgc_provinces'; // Set the name of the database table
    ...
    public function run()
    {
        ...
        for ($i = 1; $i < count($data); $i++) {
            ...
            $insertData = [
                'prov_code' => $row[0] ?? null,
                'prov_name' => $row[1] ?? null,
                'region_code' => $row[2] ?? null,
                'created_at' => now(),
                'updated_at' => now(),
            ];
