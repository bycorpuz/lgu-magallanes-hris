...
class LibPSGCCities extends Seeder
{
    protected $excelFilePath = 'public/system-db-libraries/lib_psgc_cities.xls'; // Set the path to your Excel file
    protected $table = 'lib_psgc_cities'; // Set the name of the database table
    ...
    public function run()
    {
        ...
        for ($i = 1; $i < count($data); $i++) {
            ...
            $insertData = [
                'city_code' => $row[0] ?? null,
                'city_name' => $row[1] ?? null,
                'prov_code' => $row[2] ?? null,
                'created_at' => now(),
                'updated_at' => now(),
            ];
